...
def calculate_recall(result_stat, iou):
    """
    input: 1. result_stat (fp, tp, gt numbers) 2. iou
    return: 1. recall 2. recall values list for each detection
    """
    iou_data = result_stat[iou]
    tp = np.array(iou_data['tp'])
    gt_total = iou_data['gt']

    tp_cumsum = np.cumsum(tp)

    cumulative_rec = tp_cumsum/gt_total if gt_total > 0 else [0]*len(tp)
    recall = cumulative_rec[-1] if len(cumulative_rec) > 0 else 0.0

    return recall, cumulative_rec
...
def eval_final_results(result_stat, save_path, infer_info=None):
    ...
    rec_30, cumrec30 = calculate_recall(result_stat, 0.30)
    rec_50, cumrec50 = calculate_recall(result_stat, 0.50)
    rec_70, cumrec70 = calculate_recall(result_stat, 0.70)
    ...
    print('The Average Recall at IOU 0.3 is %.5f, ' #used to be %.2f, for the purpose of clear comparison
          'The Average Recall at IOU 0.5 is %.5f, '
          'The Average Recall at IOU 0.7 is %.5f' % (rec_30, rec_50, rec_70))
