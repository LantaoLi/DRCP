...
class PointPillarPyramidLoss(PointPillarDepthLoss):
    ...
    def forward_collab(self, output_dict, target_dict, suffix):
        ...
        """fm or diffusion occ loss"""
        if 'fm_occ' in output_dict:
            fm_occ_loss = self.single_size_calc_occ_loss(output_dict['fm_occ'], positives, negatives, batch_size)
            print("fm_occ_loss: " + str(fm_occ_loss))
            occ_loss += fm_occ_loss*0.1
        total_loss = occ_loss
        self.loss_dict = {
            'pyramid_loss': occ_loss.item(),
            'total_loss': total_loss.item()
    ...
    def calc_occ_loss(self, occ_single_list, positives, negatives, batch_size):
        total_occ_loss = 0
        """previous occ_positives and occ_negatives"""
        ...
        """updated occ_positives and occ_negatives"""
        """
        occ_positives = positives[...,0]
        for i in range(int(positives.shape[-1])-1):
            occ_positives = torch.logical_or(occ_positives, positives[...,i+1]).float()
        occ_negatives = negatives[...,0]
        for i in range(int(negatives.shape[-1])-1):
            occ_negatives = torch.logical_and(occ_negatives, negatives[...,i+1]).float()
        occ_positives = occ_positives.unsqueeze(-1)
        occ_negatives = occ_negatives.unsqueeze(-1)
        """

        config_length = len(self.relative_downsample)
        ...
        for i, occ_preds_single in enumerate(occ_single_list):
            ...
            """
            scale_i = i if i < 3 else 0
            #print("occ loss round at:" + str(i))
            #print("but occ scale at:" + str(scale_i))

            positives_level = F.max_pool2d(occ_positives.permute(0,3,1,2), kernel_size=self.relative_downsample[scale_i]).permute(0,2,3,1)
            negatives_level = 1 - F.max_pool2d((1 - occ_negatives).permute(0,3,1,2), kernel_size=self.relative_downsample[scale_i]).permute(0,2,3,1)
            ...
            occ_loss *= self.pyramid_weight[i%config_length]

            total_occ_loss += occ_loss

        return total_occ_loss

    def single_size_calc_occ_loss(self, occ_preds_single, positives, negatives, batch_size):
        total_occ_loss = 0
        occ_positives = torch.logical_or(positives[...,0], positives[...,1]).unsqueeze(-1).float() # N, H, W
        occ_negatives = torch.logical_and(negatives[...,0], negatives[...,1]).unsqueeze(-1).float() # N, H, W
        """
