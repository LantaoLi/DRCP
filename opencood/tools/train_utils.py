...
def load_saved_difmodel(saved_path, model):
    """
    Load saved model if exiseted
    ...
    def findLastCheckpoint(save_dir):
        ...
        if file_list:
            ...
            initial_epoch_ = 0
        return initial_epoch_
    file_list = glob.glob(os.path.join(saved_path, 'dif_parial_net_epoch_bestval_at*.pth'))
    if file_list:
        assert len(file_list) == 1
        print("resuming best validation model at epoch %d" % \
                eval(file_list[0].split("/")[-1].rstrip(".pth").lstrip("dif_parial_net_epoch_bestval_at")))
        loaded_state_dict = torch.load(file_list[0] , map_location='cpu')
        check_missing_key(model.state_dict(), loaded_state_dict)
        model.load_state_dict(loaded_state_dict, strict=False)
        return eval(file_list[0].split("/")[-1].rstrip(".pth").lstrip("dif_parial_net_epoch_bestval_at")), model

    initial_epoch = findLastCheckpoint(saved_path)
    if initial_epoch > 0:
        print('resuming by loading epoch %d' % initial_epoch)
        loaded_state_dict = torch.load(os.path.join(saved_path,
                         'dif_parial_net_epoch%d.pth' % initial_epoch), map_location='cpu')
        check_missing_key(model.state_dict(), loaded_state_dict)
        model.load_state_dict(loaded_state_dict, strict=False) #was strict = False
...
def load_merged_model(saved_path, model):
    ...
    def findLastCheckpoint(save_dir):
        ...
        if file_list:
            ...
            initial_epoch_ = 0
        return initial_epoch_
    file_list = glob.glob(os.path.join(saved_path, 'dif_merge_net.pth')) # default was 'dif_merge_net.pth'
    if file_list:
        assert len(file_list) == 1
        print("Finding merged model!")
        loaded_state_dict = torch.load(file_list[0] , map_location='cpu')
        check_missing_key(model.state_dict(), loaded_state_dict)
        model.load_state_dict(loaded_state_dict, strict=False)
        return 0, model
...
def setup_diffuse_optimizer(hypes, model):
    """
    Create optimizer corresponding to the yaml file
    ...
    method_dict = hypes['optimizer']
    optimizer_method = getattr(optim, method_dict['core_method'], None)
    # select the paras U want
    """
    train_parameters = list(model.diffusion_unet.parameters())
    train_parameters += list(model.difmasker.parameters())
    train_parameters += list(model.diffusew.parameters())
    #train_parameters += list(model.cross_fusion.parameters())
    #train_parameters += list(model.pyramid_backbone.parameters())
    train_parameters += list(model.shrink_conv.parameters())
    train_parameters += list(model.cls_head.parameters())
    train_parameters += list(model.reg_head.parameters())
    train_parameters += list(model.dir_head.parameters())
    """
    #train all paras of the whole model
    if 'end2end' not in method_dict or method_dict['end2end'] == True:
        train_parameters = model.parameters()
    else:
        if method_dict['target_module'] == 'fm_net':
            train_parameters = list(model.fm_net.parameters())
        elif method_dict['target_module'] == 'dif':
            train_parameters = list(model.diffusion_unet.parameters())
            train_parameters += list(model.difmasker.parameters())
            train_parameters += list(model.diffusew.parameters())
        #other_parameters = list(set(model.parameters()) - set(tr))
    """
    difmasker_params = list(model.difmasker.parameters())
    other_params = list(set(model.parameters()) - set(difmasker_params))
    """
    if not optimizer_method:
        raise ValueError('{} is not supported'.format(method_dict['name']))
    if 'args' in method_dict:
        return optimizer_method(train_parameters,
                                lr=method_dict['lr'],
                                **method_dict['args'])
        """
        print("differentiated learning rate!!!!!")
        return optimizer_method([
            {"params": other_params, "lr": method_dict['lr']},
            {"params": difmasker_params, "lr": method_dict['lr']}
        ], **method_dict['args'])
        """
    else:
        return optimizer_method(train_parameters,
