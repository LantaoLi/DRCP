...
"""
RG-Attn (Diffusion, PRGAF, Intrisic-calibration) for DairV2X
"""
...
from opencood.models.fuse_modules.pyramid_cross_fuse import PyramidCrossFusion
from opencood.models.fuse_modules.cross_fuse import SlicingCrossAttention, DepthAwareCrossAttention, PyramidDepthAwareCrossAttention
from opencood.models.fuse_modules.cross_fuse import UndistortedDepthAwareCrossAttention, UndistortedDepthAwareMHAConvFusion
from opencood.models.fuse_modules.cross_fuse import UndistortedDepthAwareCrossConvAttention
...
from opencood.models.sub_modules.torch_transformation_utils import \
    warp_affine_simple
from opencood.models.sub_modules.unet import LightweightUNet
from opencood.models.sub_modules.unet_condition import ConditionalUNet
from opencood.models.sub_modules.codif import ConditionedUNetDiffuser
import importlib
import torchvision
...
import os
import re
from diffusers import VQModel, AutoencoderKL, UNet2DModel, DDPMScheduler, UNet2DConditionModel, DDIMScheduler
from opencood.models.sub_modules.bev_dif_masker import BEVDiffuseMasker, BEVDiffusePerMasker
from opencood.models.sub_modules.bev_dif_masker import BEVDiffusePer2ChMasker, BEVDiffusePerMaskFuser, BEVDiffuseAttnFuser
from opencood.models.fuse_modules.final_bev_selfattn import BEVConv, BEVMultiConv, BEVDynamicMultiConv, MultiPerChaConv
...
class HeterPyramidCollabAllcross(nn.Module):
    def __init__(self, args):
        ...
        if 'type' in args['cross_fusion'] and args['cross_fusion']['type'] == 'LevelDepth':
            ...
            print("!!!loading LevelDepth Integrated!!!")
        else:
            if 'type' not in args['cross_fusion'] or args['cross_fusion']['type'] == 'UndistortedDepth':
                ...
                print("!!!loading UndistortedDepthAware!!!")
            elif 'type' in args['cross_fusion'] and args['cross_fusion']['type'] == 'UndistortedDepthConv':
                self.cross_fusion = UndistortedDepthAwareCrossConvAttention(args['cross_fusion']).cuda()
                print("!!!loading ConvUndistortedDepthAware!!!")
        ...
        self.diffuse_bev = args['diffusion']['diffuse_bev'] if 'diffusion' in args and 'diffuse_bev' in args['diffusion'] else False
        self.diffuse_train = False
        if 'diffusion' in args or self.diffuse == True:
            self.diffuse = True
            if self.diffuse_bev == True:
                ...
                self.itera_nums = 1 if 'itera_nums' not in args['diffusion'] else args['diffusion']['itera_nums']
        ...
        self.final_selfattn = BEVDynamicMultiConv() if 'final_selfattn' in args else None
        """
        Shared Heads
        """
    ...
    def forward(self, data_dict):
        ...
        if self.diffuse_bev and self.itera_nums == 1:
            for i in range(1):
                ...
                if self.diffuse_train:
                    ...
                    timesteps = torch.full((fused_feature.shape[0],), self.scheduler.num_train_timesteps-1, device=record_len.device).long()
                    print("Diffuse train mode")
                if not self.diffuse_train:
                    timesteps = torch.full((fused_feature.shape[0],), self.scheduler.num_train_timesteps-1, device=record_len.device).long()
                    print("Diffuse eval mode")
                #noise adding
                noise = torch.randn_like(fused_feature)
                noisy_bev = self.scheduler.add_noise(fused_feature, noise, timesteps)

                b, nc, h, w = masked_bev.shape
                bev_hidden_states = masked_bev.view(b, h*w, nc)

                #denoising
                predicted_bev = self.diffusion_unet.forward(noisy_bev, timesteps, bev_hidden_states).sample

                original_fused_feature = fused_feature
        ...
        #attention or conv on BEV internally for the final round?
        if self.final_attn:
            fused_feature = self.final_selfattn.forward(fused_feature)
            #fused_feature = self.final_selfattn1.forward(fused_feature)
